...
class ModYooRecipeHelper
{
    ...
    public static function getRecipes($params)
    {
		...
		$query->select( 'r.id, r.access, c.cat_id as cat_id, d.title as cat_title, r.title, r.alias, r.description, r.preparation, r.nb_persons, r.difficulty, r.cost, ' .
						'r.creation_date, r.publish_up, r.publish_down, r.preparation_time, r.cook_time, r.wait_time, r.featured, '.
						'r.picture, r.published, r.nb_views, r.note, r.price, r.language');
		...
		$query->join('INNER', '#__categories d on c.cat_id = d.id');
	...
	/**
     * getIngredientsByGroupId
	 */
	public static function getIngredientsByGroupId($group_id)
	{
		// Get a reference to the database
        $db 	= JFactory::getDBO();
		$query 	= $db->getQuery(true);
		...
		$query->select('id, recipe_id, group_id, quantity, unit, description, ordering');
		$query->from('#__yoorecipe_ingredients');
		
		$query->where('group_id = '.$db->quote($group_id));
	 ...
	 */
	public static function getIngredientsGroupsByRecipeId($recipe_id)
	{
		// Get a reference to the database
        $db 	= JFactory::getDBO();
		$query 	= $db->getQuery(true);
		...
		// from the recipe ingredients table
		$query->select('id, recipe_id, label, ordering');
