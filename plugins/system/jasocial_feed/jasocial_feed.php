...
/**
 * ------------------------------------------------------------------------
 * JA System Social Feed Plugin for Joomla 2.5 & 3.4
 * ------------------------------------------------------------------------
...
class plgSystemjasocial_feed extends JPlugin
{
	...
	private function fetchInstagramPost ($profile) {
		...
		if(count($items)) {
			...
			foreach ($items as $dt) {
				...
				$title = $this->cutTitleFromContent($dt->title);
				$tcontent = $dt->title;
	...
	private function getInstagramItems($keyword, $limit) {
		...
		if(strtoupper($keyword) == '[POPULAR]') {
			...
			$author = $username;
			$xml = ("http://iconosquare.com/feed/{$username}");
		} elseif (substr($keyword, 0, 1) == '#') {
			$keyword = substr($keyword, 1);
			$xml = ("http://iconosquare.com/tagFeed/{$keyword}");
		} else {
			$xml = ("http://iconosquare.com/tagFeed/{$keyword}");
		}
