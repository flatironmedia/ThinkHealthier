...
$html[] = '<div class="row-fluid '.$cssClass.'">';
...
if ($blog_show_rating){
	...
	if (isset($recipe->nb_reviews)) {
		$html[] = JText::sprintf('COM_YOORECIPE_NB_REVIEWS', $recipe->nb_reviews);
	}
}
...
if ($blog_show_pending_recipes) {
	...
	if ($recipe->published_up != 1 || $recipe->published_down != 0) {
		$html[] = '<img src="'.JURI::root().'.media/com_yoorecipe/images/pending.png" alt="'.htmlspecialchars($recipe->title).'" title="'.JText::_('COM_YOORECIPE_EXPIRED').'"/>';
	} else if (!$recipe->validated) {
		$html[] = '<img src="'.JURI::root().'media/com_yoorecipe/images/pending.png" alt="'.htmlspecialchars($recipe->title).'" title="'.JText::_('COM_YOORECIPE_PENDING_APPROVAL').'"/>';
	}
}
...
if ($blog_show_seasons) {
	$html[] = JHtml::_('yoorecipeutils.generateRecipeSeason', $recipe->seasons, $do_row_fluid = true);
}
...
if ($blog_show_difficulty) {
	...
	$html[] = '<div class="recipe-difficulty span6">'.JText::_('COM_YOORECIPE_RECIPES_DIFFICULTY').': ';	
...
if ($blog_show_cost) {
	$html[] = '<div class="recipe-cost span6">'.JText::_('COM_YOORECIPE_RECIPES_COST').': ';
	$html[] = '<span class="label label-warning">';
...
if ($blog_show_preparation_time && $recipe->preparation_time != 0) {
	$html[] = '<div class="recipe-preptime span5">'.JText::_('COM_YOORECIPE_YOORECIPE_PREPARATION_TIME_LABEL').': '.JHtml::_('datetimeutils.formattime', $recipe->preparation_time).'</div>';
}

if ($blog_show_cook_time && $recipe->cook_time != 0) {
	$html[] = '<div class="recipe-cooktime span3">'.JText::_('COM_YOORECIPE_RECIPES_COOK_TIME').': '.JHtml::_('datetimeutils.formattime', $recipe->cook_time).'</div>';
}

if ($blog_show_wait_time && $recipe->wait_time != 0) {
	$html[] = '<div class="recipe-waittime span3">'.JText::_('COM_YOORECIPE_RECIPES_WAIT_TIME').': '.JHtml::_('datetimeutils.formattime', $recipe->wait_time).'</div>';		
}
$html[] ='</div>';		
