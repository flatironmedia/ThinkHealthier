...
if ($recipe->kcal != 0) {
	$html[] = '<div style="position:relative"><span class="calories-item">'.$recipe->kcal.' '.JText::_('COM_YOORECIPE_YOORECIPE_KCAL_LABEL').'</span></div>';
}

$image_html = '<img class="thumbnail" src="'.JURI::root().$picture_path.'" alt="'.htmlspecialchars($recipe->title).'" title="'.htmlspecialchars($recipe->title).'" />';
