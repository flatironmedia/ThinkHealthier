 ...
 * YooRecipe Model
 */
class YooRecipeModelLandingPage extends JModelList
{
	...
	protected function getListQuery($categoryId = null, $orderBy = null)
	{
		...
		$query->select('CASE WHEN fr.recipe_id = r.id AND fr.user_id = '. $db->quote($user->id).' THEN 1 ELSE 0 END as favourite');
	...
	* getAtoZRecipes
	*/
	public function getAtoZRecipes()
	{
		$result = array();
		...
		// From the recipe table
		$query->select('distinct(upper(left(r.title, 1))) as letter');
