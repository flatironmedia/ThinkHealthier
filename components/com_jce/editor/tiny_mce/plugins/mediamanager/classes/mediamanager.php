...
class WFMediaManagerPlugin extends WFMediaManager {
    ...
    var $_filetypes = 'windowsmedia=avi,wmv,wm,asf,asx,wmx,wvx;quicktime=mov,qt,mpg,mpeg,m4a;flash=swf;shockwave=dcr;real=rm,ra,ram;divx=divx;video=mp4,ogv,ogg,webm;audio=mp3,ogg,wav;silverlight=xap';
    ...
    public function getFileDetails($file) {
        ...
        if ($filesystem->get('local')) {
            ...
            if (preg_match('/\.(xml)/i', $file)) {
                ...
                if (preg_match('/\.(mp3|wav|aiff|oga)/i', $file)) {
                    $width = $height = '';
                }
            }
