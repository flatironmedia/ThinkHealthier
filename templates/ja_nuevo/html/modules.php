...
{ 
	...
	if (!empty ($module->content)) {
		$html = "<{$moduleTag} class=\"t3-module row-feature {$moduleClassSfx} {$moduleClass}\" id=\"Mod{$module->id}\"><div class=\"container\">". $badge;

		if ($module->showtitle != 0) {
			$html .= "<{$headerTag} class=\"row-feature-title {$headerClass}\"><span>{$module->title}</span></{$headerTag}>";
		}

		$html .= "<div class=\"row-feature-ct\">{$module->content}</div></div></{$moduleTag}>";
