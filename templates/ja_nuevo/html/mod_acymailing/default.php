...
?>
<div class="acymailing_<?php echo $params->get('displaymode','vertical');?> acymailing_module<?php echo $params->get('moduleclass_sfx')?>" id="acymailing_module_<?php echo $formName; ?>">
<?php
    //Effect mootools-slide
    ...
    //End effect mootools-slide
	//Check one field
	$conefield = '';
	if(count($fieldsToDisplay) == 1) $conefield=" onefield";
	?>

	<div class="acymailing_fulldiv<?php echo $conefield;?>" id="acymailing_fulldiv_<?php echo $formName; ?>" <?php echo ($params->get('textalign','none') != 'none') ? '' : 'style="'.$params->get('textalign','none').'"'; ?> >
		<form id="<?php echo $formName; ?>" action="<?php echo JRoute::_('index.php'); ?>" onsubmit="return submitacymailingform('optin','<?php echo $formName;?>')" method="post" name="<?php echo $formName ?>" <?php if(!empty($fieldsClass->formoption)) echo $fieldsClass->formoption; ?> >
			...
			<?php
                if(!empty($introText)) :
                    echo '<div class="acymailing_introtext">'.$introText.'</div>';
                endif;
            ?>
			...
			<?php if(!empty($visibleListsArray) && $listPosition == 'before'): ?>
				...
				<?php if($params->get('dropdown',0)){ ?>
                    <div class="col-md-12 listpositionbefore">
			    ...
			    <?php }//endif dropdown ?>
			<?php endif;//endif visiblelists ?>

            <!-- Acymailing form - convert used div -->
			<div class="acymailing_form">
                <?php
                $rowclass = "col-md-".(12/count($fieldsToDisplay));
                if(!$displayInline) $rowclass="";

                foreach($fieldsToDisplay as $oneField){
                    if($oneField == 'name' AND empty($extraFields[$oneField])){ ?>
                     
                            <?php if($displayOutside): ?>
                                <label for="user_name_<?php echo $formName;?>"><?php echo $nameCaption;?></label>
                            <?php endif; ?>

                            <div class="acyfield_<?php echo $oneField; ?>">
                                <input id="user_name_<?php echo $formName; ?>" <?php if(!empty($identifiedUser->userid)) echo 'disabled="disabled" ';  if(!$displayOutside){ ?> onfocus="if(this.value == '<?php echo $nameCaption;?>') this.value = '';" onblur="if(this.value=='') this.value='<?php echo $nameCaption?>';"<?php } ?> class="inputbox form-control" type="text" name="user[name]" style="width:<?php echo $fieldsize; ?>" value="<?php if(!empty($identifiedUser->userid)) echo $identifiedUser->name; elseif(!$displayOutside) echo $nameCaption; ?>" />
                            </div>
                    ...
                    <?php
                            ...
                            <?php if($displayOutside): ?>
                                <label for="user_name_<?php echo $formName;?>"><?php echo $emailCaption;?></label>
                            <?php endif; ?>

                            <div class="acyfield_<?php echo $oneField; ?>">
                                <input id="user_email_<?php echo $formName; ?>" <?php if(!empty($identifiedUser->userid)) echo 'disabled="disabled" ';  if(!$displayOutside){ ?> onfocus="if(this.value == '<?php echo $emailCaption;?>') this.value = '';" onblur="if(this.value=='') this.value='<?php echo $emailCaption?>';"<?php } ?> class="inputbox form-control" type="text" name="user[email]" style="width:<?php echo $fieldsize; ?>" value="<?php if(!empty($identifiedUser->userid)) echo $identifiedUser->email; elseif(!$displayOutside) echo $emailCaption; ?>" />
                            </div>
                       
                    <?php
                    }elseif($oneField == 'html' AND empty($extraFields[$oneField])){
                        echo '<div class="acyfield_'.$oneField.' '.$rowclass.'">';
                        echo '<span>'.JText::_('RECEIVE').'</span>';
                       ...
                       <?php if($displayOutside){
                            echo '<div class="displayOutside"><label '.((strpos($extraFields[$oneField]->type,'text') !== false) ? 'for="user_'.$oneField.'_'.$formName.'"' : '' ).'>'.$fieldsClass->trans($extraFields[$oneField]->fieldname).'</label></div>';
                        }
                        //Size of input
                        $sizestyle = '';
                ...
                ?>
                    ...
                    <?php if(empty($identifiedUser->userid) AND $config->get('captcha_enabled') AND acymailing_level(1)){ ?>
                        <div class="captchakeymodule col-md-12">
                        <?php
                        if(ACYMAILING_J16){
					...
					<?php if(!empty($visibleListsArray) && $listPosition == 'after'){
						if($params->get('dropdown',0)){ ?>
                           <div class="col-md-12 listpositionafter">
							<select name="subscription[1]">
								<?php foreach($visibleListsArray as $myListId){?>
								<option value="<?php echo $myListId ?>"><?php echo $allLists[$myListId]->name; ?></option>
								<?php } ?>
						...
						<?php }else{ ?>
                            ...
                            <div class="acymailing_lists">
                                <?php foreach($visibleListsArray as $myListId){
                                    ...
                                    ?>
                                        <div class="acymailing_visible_lists">
                                            <input type="checkbox" class="acymailing_checkbox" name="subscription[]" id="acylist_<?php echo $myListId; ?>" <?php echo $check; ?> value="<?php echo $myListId; ?>"/>
                                            <label for="acylist_<?php echo $myListId; ?>">
                                            <?php
                                            $joomItem = $params->get('itemid',0);
					...
					<div class="acysubbuttons">
						...
						<button class="button unsubbutton  btn btn-inverse" type="button" value="<?php echo $params->get('unsubscribetext',JText::_('UNSUBSCRIBECAPTION')); ?>" name="Submit" onclick="return submitacymailingform('optout','<?php echo $formName;?>')">
						</button>
			...
			<?php $myItemId = $config->get('itemid',0);
                  if(empty($myItemId)){ global $Itemid; $myItemId = $Itemid;}
                  if(!empty($myItemId)){ ?>
	...
	<?php
    //Effect mootools-slide close div
    if($params->get('effect','normal') == 'mootools-slide'){ ?>
        </div>
    <?php }
    //End Effect mootools-slide
