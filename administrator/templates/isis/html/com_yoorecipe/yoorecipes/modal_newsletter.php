...
foreach ($this->items as $i => &$item) {
	$db = JFactory::getDbo();
	$query = "SELECT * FROM `#__newsletter_history` WHERE type=2 AND article_recipe_id=".$item->id." ORDER BY timestamp DESC";
	$db->setQuery($query);
	$results = $db->loadObjectList();

	$item->last_used_date = substr($results[0]->timestamp, 0, 10);
	//$item->last_used_position = $results[0]->position;
	if (empty($item->last_used_date)) $item->last_used_date = "never";
}

if (isset($_GET["field_name"])) if (!(empty($_GET["field_name"]))) $_SESSION["field_name"] = $_GET["field_name"];
if (isset($_GET["field_id"])) if (!(empty($_GET["field_id"]))) $_SESSION["field_id"] = $_GET["field_id"];


//die('<pre>'.print_r($this->items, true).'</pre>');
...
<form action="<?php echo JRoute::_('index.php?option=com_yoorecipe&layout=modal_newsletter&tmpl=component'); ?>" method="post" name="adminForm" id="adminForm">
		...
		<table class="table table-striped" id="recipeList">
			<thead>
				<th width="1%" class="nowrap hidden-phone">
					<?php echo JHtml::_('grid.sort', 'COM_YOORECIPE_YOORECIPE_HEADING_ID', 'r.id', $listDirn, $listOrder); ?>
				</th>
				<!-- <th width="1%" class="nowrap hidden-phone">
			...
			?>
				<tr class="row<?php echo $i % 2; ?>">
					...
					<!-- <?php if ($item->checked_out) : ?>
						<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'yoorecipes.', $canCheckin); ?> 
					<?php endif; ?>-->
						<a href="javascript:void(0)" onclick="if (window.parent) window.parent.<?php echo $this->escape($function);?>('<?php echo $item->id; ?>', '<?php echo $this->escape(addslashes($item->title)); ?>', '<?php echo $this->escape($item->catid); ?>', '<?php echo $_SESSION["field_name"]; ?>', '<?php echo $_SESSION["field_id"]; ?>', '<?php echo $this->escape($lang); ?>', null);"><?php echo $item->title; ?></a>
						<p class="smallsub"><?php echo JText::sprintf('JGLOBAL_LIST_ALIAS', $this->escape($item->alias));?></p>
					</td>
					<!-- <td class="nowrap hidden-phone"><?php echo JHtml::_('yoorecipeadminutils.featured', $item->featured, $i, false, 'yoorecipes.'); ?></td> -->
					...
					<td class="nowrap hidden-phone">
						<?php echo JHtml::_('datetimeutils.formattime', $item->preparation_time);	?>
					</td>
					<td class="nowrap hidden-phone">
						<?php echo JHtml::_('datetimeutils.formattime', $item->cook_time); ?>
					</td>
					<td class="nowrap hidden-phone">
						<?php echo JHtml::_('datetimeutils.formattime', $item->wait_time); ?>
					</td>
					<td class="nowrap hidden-phone">
