...
class YooRecipeModelCategories extends JModelList
{
	...
	*/
	...
	protected function populateState($ordering = 'ordering', $direction = 'ASC')
	{
		...
		$recipes_order = $yooRecipeparams->get('recipes_order', 'title');
		
		switch ($recipes_order) {
			default:
			case 'title':
				$this->setState('orderCol', 'r.title asc');
			break;
			case 'random':
				$this->setState('orderCol', 'rand()');
			break;
			case 'featured':
				$this->setState('orderCol', 'r.featured desc');
			break;
			case 'mostviewed':
				$this->setState('orderCol', 'r.nb_views desc');
			break;
			case 'bestrated':
				$this->setState('orderCol', 'r.note desc');
			break;
			case 'creation_date':
				$this->setState('orderCol', 'r.creation_date desc');
			break;
		}
	...
	protected function getListQuery()
	{
		...
		// Select some fields
		$query->select('SQL_CALC_FOUND_ROWS r.id, r.access, r.title, r.alias, r.description, r.created_by, r.preparation, r.serving_type_id' .
				...
				', r.validated, r.featured, r.nb_views, r.note, r.price, r.language');
		...
		$query->order($this->getState('orderCol'));
		return $query;
	}
	...
	public function getCategoryById($categoryId)
	{
		...
		// Select some fields
		$query->select('id, title, alias, language, access, description, metadesc, metakey, metadata, published');
		$query->select('CASE WHEN CHARACTER_LENGTH(alias) THEN CONCAT_WS(\':\', id, alias) ELSE id END as slug');
	...
	public function getCategoriesByParentId($parentSlug) {
		...
		$query->order('lft'.' '.'asc');
		//--- Xander@OGOSense Limit Categories to 9 ---//
	 ...
	 * @param category: The category id to look for
	 * @param recursive: Whether or not children categories should be included in count
	 */
	public function getNbRecipesByCategoryId($cat_slug, $recursive = true) {
		...
		// Prepare where clause
		$query->where('jr.published = 1');
		$query->where('jr.validated = 1');
		
		$query->where('jr.language in (' . $db->quote(JFactory::getLanguage()->getTag()) . ',' . $db->quote('*') . ')');
		
		$slug_chunks = preg_split('/:/',$cat_slug);
		$sub_cat_ids = $this->getAllSubCategories($slug_chunks[0], $recursive = true, $force = true);
		if (count($sub_cat_ids) > 0) {
			$query->where('cc.cat_id IN ('.$slug_chunks[0].','.implode(',', $sub_cat_ids).')');
		} else {
			$query->where('cc.cat_id = '.$slug_chunks[0]);
		}
