...
class YooRecipeTableYooRecipe extends JTable
{
	...
	function __construct(&$db) 
	{
		...
		JTableObserverTags::createObserver($this, array('typeAlias' => 'com_yoorecipe.recipe'));
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_yoorecipe.recipe'));
	}
	...
	function check()
	{
		...
		if (isset($this->creation_date)) {
			$date = new JDate($this->creation_date);
			$this->creation_date = $date->toSQL(true);
		}
		else {
		...
		if (!isset($jform['creation_date'])) {
			$jform['creation_date'] = JFactory::getDate()->format('Y-m-d H:i:s');
		}
		...
		$this->price			= (isset($jform['price'])) ? str_replace(",", ".", $jform['price']) : '';
		$this->sugar			= (isset($jform['sugar'])) ? str_replace(",", ".", $jform['sugar']) : '';
		$this->carbs			= (isset($jform['carbs'])) ? str_replace(",", ".", $jform['carbs']) : '';
		$this->fat				= (isset($jform['fat'])) ? str_replace(",", ".", $jform['fat']) : '';
		$this->saturated_fat	= (isset($jform['saturated_fat'])) ? str_replace(",", ".", $jform['saturated_fat']) : '';
		$this->cholesterol		= (isset($jform['cholesterol'])) ? str_replace(",", ".", $jform['cholesterol']) : '';
		$this->proteins			= (isset($jform['proteins'])) ? str_replace(",", ".", $jform['proteins']) : '';
		$this->fibers			= (isset($jform['fibers'])) ? str_replace(",", ".", $jform['fibers']) : '';
		$this->salt				= (isset($jform['salt'])) ? str_replace(",", ".", $jform['salt']) : '';
		
		// Manage default nb_persons
		if (!isset($jform['nb_persons'])) {
			$this->nb_persons = 4;
		}
	...
	public function validate($pks = null, $state = 1, $userId = 0)
	{
		...
		if ($state && $yooRecipeparams->get('notify_on_validate', 0)) {
			...
			foreach ($pks as $pk) {
				...
				// Select some fields
				$query->select('r.id, r.title, r.alias, r.description, r.created_by, r.category_id, c.title as category, r.preparation, r.nb_persons, r.difficulty, r.cost, r.creation_date, r.preparation_time, r.cook_time, r.wait_time, r.picture, r.published, r.validated, r.featured, r.nb_views, r.note');				
				...
				JHtmlEmailUtils::sendMailToUserOnValidation($recipe);
			}
		}
