<?php
/* Adjusted for use in Feed Gator by Matt Faulds. See "//FG edit" Last edit FG2.3.7 */
/**
...
class Readability
{
	...
	public $articleImages; // added to hold images FG edit
	public $dom;
	...
	**/
	public $regexps = array(
        'likelyCandidates' => '/photos|images|gallery|x/i',
	...
	const FLAG_GRAB_LIKELYS = 1; //get likely images
	...
	**/
	public function init()
	{
		...
		$relaxParsing	= $fgParams->get('relax_parsing'); //FG edit
	...
	**/
	protected function grabImages($page=null) {
		...
		$grabLikelyCandidates = $this->flagIsActive(self::FLAG_GRAB_LIKELYS);
	...
	//	$nodesToScore = array();
		...
		//for ($nodeIndex=$targetList->length-1; $nodeIndex >= 0; $nodeIndex--) {
			...
			if ($grabLikelyCandidates) {
				...
				if (
					preg_match($this->regexps['likelyCandidates'], $likelyMatchString) &&
					$tagName != 'BODY'
				)
				{
					if($likelyCandidates = $fgParams->get('likelyCandidates','')) {
						$likelyCandidates = explode(',',$likelyCandidates);
						$this->regexps['likelyCandidates'] = str_replace('x/i',implode('|',$likelyCandidates).'/i',$this->regexps['likelyCandidates']);
					} else {
						$this->regexps['likelyCandidates'] = str_replace('|x/i','/i',$this->regexps['likelyCandidates']);
					}
